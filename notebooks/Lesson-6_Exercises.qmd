...
El tamaño muestral necesario cumple:
N ≥ Var[θ] / (error máximo)^2
N ≥ 0.0833 / (0.01)^2 = 833
Por seguridad, usamos N = 1000 simulaciones.
...
```
La distribución previa Beta(1,1) se ha aproximado con 1000 muestras de Monte Carlo, obteniendo una media de 0.493 y varianza de 0.0805, consistentes con los valores teóricos (0.5 y 0.0833). El error estándar de 0.009 cumple con la precisión requerida (≤ 0.01)
:::
...
```
La probabilidad de obtener un valor igual o menor que 16 éxitos bajo la distribución predictiva previa es del 79.1%. Esto significa que, en el 79.1% de las simulaciones de la distribución predictiva previa, el número de éxitos es menor o igual a 16.
Por otro lado, hay una probabilidad del 26.3% de obtener un número de éxitos igual o mayor que 16 en las simulaciones.
...
theta_post_mc <- rbeta(length(theta_mc), 17, 7)
...
```
La distribución posterior de θ es una Beta(17,7) con media 0.708 e intervalo de credibilidad 95% [0.523, 0.871]. Esto sugiere una tasa de aceptación estimada del 71%. La distribución predictiva posterior ahora explica de manera más precisa los datos, a diferencia de la predictiva previa. La posterior Beta(17,7) refleja el aprendizaje a partir de los datos, moviendo la estimación inicial de θ (que era del 50% con la previa Beta(1,1)) a un 71%.
...
set.seed(20250327)  # Para reproducibilidad
...
```
Hay aproximadamente un 38.6% de probabilidad de observar 16 o más éxitos en los 22 ensayos simulados.
:::
...
y_rep_post_1 <- rbinom(10000, size = 22, prob = rbeta(10000, 17, 7))

# Predictiva a partir de Muestra 2 (Beta(66,49)), pero para n=22
y_rep_post_2_rescaled <- rbinom(10000, size = 22, prob = rbeta(10000, 66, 49))

# Datos para el gráfico
df_comparacion <- tibble(
  Muestra = rep(c("Post-Muestra 1", "Post-Muestra 2"), each = 10000),
...
ggplot(df_comparacion %>% filter(Muestra == "Post-Muestra 2")) +
  ...
  # Área ≥16
  geom_bar(
    data = ~.x %>% filter(Exitos >= 16),
    aes(x = Exitos, y = after_stat(prop)), 
    fill = color_defecto, alpha = 0.5, width = 0.8
  ) +
  # Área ≤16
  geom_bar(
    data = ~.x %>% filter(Exitos <= 16),
    aes(x = Exitos, y = after_stat(prop)), 
    fill = color_defecto, alpha = 0.3, width = 0.8
  ) +
  geom_vline(xintercept = 16, color = color_defecto, linetype = "dashed", linewidth = 1) +
  annotate("text", x = 18, y = 0.04, 
           label = paste("P(≥16) =", round(p_mayor_igual, 2)), 
           color = color_defecto) +
  annotate("text", x = 14, y = 0.04, 
           label = paste("P(≤16) =", round(p_menor_igual, 2)), 
           color = color_defecto) +
  labs(
...
```{r}
# Simulación de la predictiva posterior para n=22 bajo el modelo de Muestra 2
set.seed(20250327)
theta_muestra2 <- rbeta(100000, 66, 49)  # Posterior de Muestra 2
y_rep_muestra2 <- rbinom(100000, size = 22, prob = theta_muestra2)

# Valor-p (dos colas): Probabilidad de desviación igual o mayor que la observada
valor_p <- mean(abs(y_rep_muestra2 - mean(y_rep_muestra2)) >= abs(16 - mean(y_rep_muestra2)))
cat("Valor-p predictivo posterior (dos colas):", round(valor_p, 4), "\n")
```
Dado que el valor-p es mayor que 0.05, no podemos rechazar la hipótesis de que las dos muestras provienen de la misma población. 
:::
...
ggplot(df_posterior, aes(x = lambda, color = grupo, fill = grupo)) +
...
```
```{r}
# Además, podemos calcular estadísticos descriptivos:
df_predictiva %>%
  group_by(grupo) %>%
  summarise(
    Media = mean(n_hijos),
    Mediana = median(n_hijos),
    SD = sd(n_hijos),
    Q25 = quantile(n_hijos, 0.25),
    Q75 = quantile(n_hijos, 0.75),
    Max = max(n_hijos)
  )
```
...
prob_igual <- mean(mujer_con == mujer_sin)
prob_menor <- mean(mujer_con < mujer_sin)

cat(
  "P(mujer con título > mujer sin título): ", prob_mayor, "\n",
  "P(igual número de hijos): ", prob_igual, "\n",
  "P(mujer con título < mujer sin título): ", prob_menor, "\n"
)
```
Los resultados nos indican que:
La probabilidad de que la mujer con título tenga más hijos que la mujer sin título es 30.1%.
La probabilidad de que ambas mujeres tengan el mismo número de hijos es 21.8%.
La probabilidad de que la mujer sin título tenga más hijos que la mujer con título es 48.1%.
Esta última es la probabilidad más alta, lo que significa que es más probable que la mujer sin título universitario tenga más hijos que la mujer con título universitario, si tomamos dos mujeres al azar.
...
```{r}
# Simulamos 1 millón de mujeres de cada grupo
set.seed(20250327) # Para reproducibilidad
n_sim <- 1e6
...
df_mujeres <- tibble(
  n_hijos = c(mujeres_con, mujeres_sin),
  titulada = rep(c(TRUE, FALSE), each = n_sim)
)

# Calculamos proporciones observadas
prop_tituladas <- df_mujeres %>%
  group_by(n_hijos) %>%
  summarise(
    n_total = n(),
    n_tituladas = sum(titulada),
    prop_tituladas = n_tituladas/n_total
  ) %>%
  filter(n_total > 100) # Filtramos para tener suficiente muestra

ggplot(prop_tituladas, aes(x = n_hijos, y = prop_tituladas, color = "Proporción tituladas")) +
  geom_point(size = 3) +  # Agregamos puntos para cada valor observado
  geom_smooth(method = "loess", formula = y ~ x, span = 0.5, se = FALSE) +  # Curva suavizada
  labs(title = "Proporción de mujeres tituladas universitarias por número de hijos",
       subtitle = "Con distribución predictiva posterior (1,000,000 simulaciones)",
       x = "Número de hijos",
...
ggplot(df_maximos, aes(x = Grupo, y = Valor, fill = Tipo)) +
  ...
  labs(title = "Comparación del máximo número de hijos observado",
       ...
       fill = "Distribución")
```
En el grupo de mujeres con título universitario, los datos empíricos muestran un máximo de 4 hijos, mientras que el modelo predictivo genera valores máximos de hasta 11 hijos. En cuanto al grupo sin título universitario, los datos empíricos muestran un máximo de 6 hijos, pero las simulaciones predictivas alcanzan hasta 12 hijos. 
...
print(max_1)
```
Este valor representa el número más alto de hijos observado en una muestra de 100 mujeres simuladas, con una tasa promedio de fertilidad de 2.5 hijos por mujer.Es decir, al menos una mujer en esta muestra simulada tuvo 9 hijos (un número bastante alto...)
:::
...
```{r}
# Extraemos el tamaño muestral del grupo con título universitario
n_con <- fert_estadisticos |> 
  filter(titulo_uni == "con") |> 
  pull(n)

max_hijos_emp <- max_empirico_con  # Máximo empírico del grupo con título

# Número de simulaciones
n_sim <- 10000  

# Simulación de máximos
maximos_simulados <- replicate(n_sim, {
  lambda_sim <- rgamma(1, shape = a_post_con, rate = b_post_con)
  max_poisson(lambda_sim, n_con)
})

# Cálculo del valor-p predictivo posterior
valor_p <- mean(maximos_simulados <= max_hijos_emp)
print(valor_p)
```
El valor-p predictivo posterior es: 0.429
...
```{r}
# Máximo observado en datos empíricos (grupo con título)
max_hijos_emp <- fertilidad_gss_1990 |> 
  filter(titulo_uni == "con", frecuencia > 0) |> 
  pull(n_hijos) |> 
  max()

# Simulación de la distribución del máximo
n_sim <- 10000
maximos_simulados <- numeric(n_sim)

for(i in 1:n_sim) {
  # 1. Simular lambda de la posterior
  lambda_sim <- rgamma(1, shape = a_post_con, rate = b_post_con)
  
  # 2. Simular muestra de tamaño n_con
  # 3. Calcular máximo
  maximos_simulados[i] <- max_poisson(lambda_sim, n_con)
}

# Cálculo del valor-p
valor_p <- mean(maximos_simulados <= max_hijos_emp)

cat("El valor-p predictivo posterior es: ", valor_p, "\n")
```
Un 43.5% de las muestras simuladas bajo el modelo tienen un máximo de hijos igual o menor que el observado en los datos reales.Esto sugiere que el máximo observado es compatible con las predicciones del modelo.
:::
