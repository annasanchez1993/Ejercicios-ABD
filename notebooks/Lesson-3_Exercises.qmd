...
En la distribución normal $N(0,1)$, el valor máximo de la densidad ocurre en $x=0$.
:::
...
alpha_values <- c(0.5, 2, 5, 3)
beta_values <- c(0.5, 5, 1, 3)
par(mfrow=c(2,2))
...
Cuando Beta(2,5): la distribución se inclina hacia la izquierda (hacia 0), porque β>α
Cuando beta(5,1): la distribución se inclina fuertemente hacia la derecha (hacia 1), porque α>β
:::
...
Cuando α y β son iguales y crecen la distribución es simétrica alrededor de 0.5 y la curva adquiere una forma de "montaña" cada vez más plana en la cima. Por ejemplo Beta (3,3). Si los parámetros son diferentes, la distribución Beta se vuelve asimétrica, inclinándose hacia un lado.
:::
...
En comparación con Beta(1,1), ya no es completamente plana, si no que comienza a formar una ligera montaña en el centro.
:::
...
La distribución Beta toma una forma de "U". Por ejemplo en Beta (0.5,0.5).
:::
...
#Definimos la precisión de los valores de p
prec <- 1e-3  

# Generamos una secuencia de valores entre 0 y 1 con incrementos de 'prec'
...
¿Y qué habrías concluido de la muestra poblacional? La tasa de aceptación más probable para la aplicación se estima en aproximadamente 0.606 o 60.6%. Esto implica que alrededor del 60% de los participantes probablemente descargarían la aplicación.
:::
...
ggplot() +
  ...
  geom_line(aes(x = rango, y = distribucion_previa), linewidth = 1.2, alpha = 0.7) +
  # Gráfico de la distribución posterior
  geom_line(aes(x = rango, y = distribucion_posterior_1), linewidth = 1.2, alpha = 0.7) +
  # Agregar líneas verticales para los estimadores
  geom_vline(xintercept = estimador_esperado_previa, linetype = "dashed", color = "blue") +  # Estimador esperado previa
  geom_vline(xintercept = estimador_modal_previa, linetype = "dashed", color = "green") +    # Estimador modal previa
  geom_vline(xintercept = estimador_esperado_posterior, linetype = "dashed", color = "red") +  # Estimador esperado posterior
  geom_vline(xintercept = estimador_modal_posterior, linetype = "dashed", color = "purple") +  # Estimador modal posterior
  labs(
